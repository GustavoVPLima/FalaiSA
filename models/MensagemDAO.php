...
class MensagemDAO
{
    ...
    public static function getUnreadCount($userId, $communityId)
    {
        $sql = "SELECT COUNT(*) as nao_lidos FROM tb_chat
                WHERE id_chat_comunidade = ? 
                AND id_chat_usuario != ? 
                AND lida = FALSE";

        $result = Database::query($sql, [$communityId, $userId]);
        $row = Database::fetchAssoc($result);
        return $row['nao_lidos'] ?? 0;
    }
}
